...
export function MoreActionsMenu({ onClose, onDelete, onEdit, visible }: MoreActionsMenuProps) {
  ...
  return (
    <YStack bottom={0} left={0} position="absolute" right={0} top={0} zIndex={100}>
      ...
      <YStack
        ...
        width={240}
      >
        <MenuRow Icon={EditIcon} label={collectionsContent.menu.edit} onPress={onEdit} />
        <MenuRow Icon={TrashIcon} label={collectionsContent.menu.delete} onPress={onDelete} />
      </YStack>
    </YStack>
  );
}
